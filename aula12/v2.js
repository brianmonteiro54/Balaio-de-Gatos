...
(function(){
  ...
  function render(){
    ...
    // Conta problemas restantes (em linhas não removidas)
    let active = 0;
    rows.forEach(row => {
      if (row.dup && completed.has('duplicates')) return; // linha sumiu, não conta
      if (row.dup && !completed.has('duplicates')) active++; // duplicata ainda presente conta como 1
...
(function(){
  ...
  // 6 cartas em cada deck. Fácil → difícil (a última a virar ✓ é a mais difícil).
  const trainCards = ['🐱','🐶','🐭','🐟','🐦','🐰'];
  const testCards  = ['🐈','🐩','🐀','🐠','🦜','🐇'];

  // Acerto no treino: sobe e satura perto de 100%
  function trainAcc(e){
    return 100 * (1 - 0.82 * Math.exp(-0.11 * e));
  }
  // Acerto no teste: sobe junto, atinge pico ~22 e depois decai (overfitting)
  function testAcc(e){
    const train = trainAcc(e);
    const gap1 = Math.max(0, (e - 22) * 0.55);  // gap cresce após época 22
    const gap2 = Math.max(0, (e - 55) * 0.25);  // acelera o estrago após 55
...
// ============================================================
(function(){
  // Base logits (before softmax) for next word after "O gato preto pulou em cima do"
  const candidates = [
  ...
  function update(){
    ...
    // simulate generation: pick deterministically the top in low temp,
    // sample at higher temp (here show the top probability one for clarity but also indicate range)
    let chosen;
    if (t < 0.3){
      chosen = candidates[0].token;
    } else {
      // pick weighted random based on probs
      const r = Math.random();
      let acc = 0;
...
(function(){
  ...
  function update(){
    ...
    // assume each token unit in our split ~= 1 token approximately
    ...
    // cost approx: R$ 0.001 per token output (didactic)
    const cost = max * 0.001;
    costEl.textContent = 'R$ ' + cost.toFixed(2).replace('.', ',');
    // latency: roughly 25ms per token (didactic)
...
// ============================================================
(function(){
  // generate fixed training data + test data (so it stays consistent)
  // True relationship: weight = 0.5 + 0.5 * sqrt(age) + noise
  // age in months 0-60
  function rng(seed){
    let s = seed;
    return () => {
  ...
  // SVG coords: age 0-60 → x 50..380. weight 0-6 → y 340..40
  function toX(age){ return 50 + (age / 60) * 330; }
  function toY(w){ return 340 - Math.min(6, Math.max(0, w / 6)) * 300; }

  // Normalização: divide idade por AGE_MAX antes de erguer pra potência. Isso evita
  // que age^15 fique gigante (60^15 ~ 4.7e26) e estoure precisão numérica.
  const AGE_MAX = 60;

  // Polynomial regression: fit polynomial of degree d to training data
  // Returns coefficients [c0, c1, ..., cd] such that y = c0 + c1*xn + c2*xn^2 + ... onde xn = age/AGE_MAX
  function fitPoly(data, degree){
    ...
    // Solve (X^T X) c = X^T y using simple Gaussian elimination
    ...
    for (let i = 0; i < m; i++){
      ...
      if (Math.abs(A[i][i]) < 1e-12) continue; // singular, skip
      for (let k = i + 1; k < m; k++){
  ...
  // draw train and test points once
  ...
  function update(){
    ...
    // draw error lines from each test point to where the curve predicts
